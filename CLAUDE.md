...
**It is engineering-only.** Roadmap, launch timing, prioritization, and product/business rationale belong to the memory vault (the executive one, on iCloud) — link that decision from an initiative rather than restating it here. An initiative plan covers how an already-decided change is sequenced, never whether it should happen. See the jurisdiction table in [`AGENTS.md`](AGENTS.md).
...
**Design-sync has one shared pin.** The `docs/experience/ui/design-system/handoff/*` bundles are synced to Claude Design (`/design-sync`). Since **2026-08-14** the `bridgecircle` bundle targets a project in the shared BridgeCircle org, so the committed `.design-sync/config.json` is the pin for *everyone* — both maintainers push the same repo bundle into the same project. The old per-user `config.local.json` override is retired; delete any stale local copy you still have, since it would push into a pre-org project nobody can see. Current and legacy pins are in each bundle's [`.design-sync/NOTES.md`](docs/experience/ui/design-system/handoff/bridgecircle/.design-sync/NOTES.md). (`toss-base` still points at its original pre-org project by decision — it stays unmigrated.)
