...
import { createDirectHelpAsk } from '@/lib/help/operations'
...
const bodySchema = z
  .object({
    ...
    requestMessage: z.string().trim().min(1).max(4_000),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await createDirectHelpAsk(
      {
    ...
    Sentry.captureException(new Error('Help direct Ask request failed'), {
      tags: { scope: 'help-direct-ask' },
    })
    return NextResponse.json(
