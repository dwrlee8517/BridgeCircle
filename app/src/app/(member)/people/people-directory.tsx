...
export function PeopleDirectory({
  ...
  async function selectPerson(person: PeopleDirectoryItem) {
    ...
    try {
      ...
      setPreview({ status: 'ready', userId: person.userId, ...body })
    } catch {
    ...
    try {
      const response = await fetch('/api/connections/requests', {
        ...
        body: JSON.stringify({
          ...
          introMessage: connection.note.trim() || null,
  ...
  return (
    <div className="min-h-full bg-[var(--surface-canvas)] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      ...
      >
        {selectedPerson ? (
          <DialogContent className="top-auto bottom-0 max-h-[92dvh] max-w-none translate-y-0 overflow-y-auto rounded-b-none p-0 sm:top-1/2 sm:bottom-auto sm:max-w-xl sm:-translate-y-1/2 sm:rounded-[var(--radius-card-xl)]">
            <PeoplePreview
              person={selectedPerson}
              relationship={relationships[selectedPerson.userId] ?? selectedPerson.relationship}
              state={preview}
...
function PeopleToolbar({
  ...
  const filterCount = activePeopleFilterCount(search)
  return (
    <div className="mt-3.5">
            ...
            <button
              ...
              className={cn(
                ...
                search.scope === scope.key
                  ...
                  : 'text-[var(--grey-600)] hover:text-[var(--text-primary)]',
              )}
      ...
      {filtersOpen ? <PeopleFilters search={search} onNavigate={onNavigate} /> : null}
    </div>
  )
}
...
function PeopleFilters({
  ...
  return (
    ...
    >
      ...
      <FilterInput
        label="Class from"
        value={filters.classYearStart === null ? null : String(filters.classYearStart)}
        inputMode="numeric"
        onChange={(value) =>
          setFilters({
            ...filters,
            classYearStart: value && /^\d{4}$/.test(value) ? Number(value) : null,
          })
        }
      />
      <FilterInput
        label="Class to"
        value={filters.classYearEnd === null ? null : String(filters.classYearEnd)}
        inputMode="numeric"
        onChange={(value) =>
          setFilters({
            ...filters,
            classYearEnd: value && /^\d{4}$/.test(value) ? Number(value) : null,
          })
        }
      />
      <div className="flex items-end gap-2">
        ...
        {activePeopleFilterCount(search) > 0 ? (
          <Button
            type="button"
            variant="ghost"
            onClick={() =>
    ...
    >
      ...
      <div className="flex items-center gap-3.5 px-4 py-3.5 sm:px-5.5">
        <button
          ...
          className="absolute inset-0 cursor-pointer rounded-[inherit] outline-none focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-focus-ring"
          onClick={onSelect}
        />
  ...
  if (relationship.state === 'connected') {
    return (
      <Button
        ...
        variant="secondary"
        size="sm"
        className={cn('rounded-full', full && 'w-full')}
      >
  ...
  return (
    ...
    >
      <div className="flex items-center">
        <span className="text-xs font-bold text-[var(--text-faint)]">Profile preview</span>
        <Button
          type="button"
          variant="ghost"
          size="icon-xs"
      ...
      {evidence ? (
        <div className="mt-3.5 rounded-[13px] bg-gradient-to-b from-[#f3f8ff] to-[#eef5ff] p-3.5 shadow-[inset_0_0_0_1px_rgb(49_130_246_/_0.14)]">
          ...
          <p className="mt-2 text-[10.5px] font-medium text-[var(--text-faint)]">
      ...
      {profile?.sharedContext.length ? (
        <div className="mt-3 rounded-[13px] bg-[var(--surface-inset)] p-3.5">
          <p className="text-[11px] font-bold text-[var(--text-faint)]">You share</p>
      ...
      <div className="mt-3 rounded-[13px] bg-[var(--surface-inset)] p-3.5">
        <p className="text-[11px] font-bold text-[var(--text-faint)]">Career</p>
      ...
      <Button asChild variant="ghost" size="sm" className="mt-2 w-full text-[var(--blue-600)]">
...
function ConnectionComposer({
  ...
  const name = state ? firstName(state.person.preferredName || state.person.displayName) : ''
  return (
    <Dialog open={Boolean(state)} onOpenChange={(open) => !open && onChange(null)}>
      {state ? (
        <DialogContent className="right-0 left-auto top-0 h-dvh max-w-[92vw] translate-x-0 translate-y-0 content-start rounded-none p-6 sm:w-[440px] sm:max-w-[440px]">
          <DialogHeader>
            <DialogTitle className="text-lg font-extrabold">Connect with {name}</DialogTitle>
            <DialogDescription className="leading-relaxed">
              Your note lands in {name}’s “Waiting on you.” Connecting is mutual, and declines stay
              quiet both ways.
            </DialogDescription>
          </DialogHeader>
          <div className="mt-3">
            <p className="text-xs font-bold text-[var(--text-secondary)]">A quick hello</p>
            <div className="mt-2 flex flex-wrap gap-2">
              {[
                `Hi ${name} — great to find you through ${organizationName}.`,
                `Hi ${name} — I’d love to connect and learn more about your work.`,
              ].map((note) => (
                <button
                  key={note}
                  type="button"
                  className="rounded-full bg-[var(--surface-subtle)] px-3 py-2 text-left text-xs font-semibold text-[var(--text-secondary)] hover:bg-[var(--blue-50)]"
                  onClick={() => onChange({ ...state, note, status: 'editing' })}
                >
                  {note}
                </button>
              ))}
          ...
          <label className="mt-4 grid gap-2 text-xs font-bold text-[var(--text-secondary)]">
            Add a note <span className="font-medium text-[var(--text-faint)]">Optional</span>
            <textarea
              className="min-h-36 resize-none rounded-xl border border-[var(--border-subtle)] bg-white p-3 text-sm font-medium outline-none focus-visible:border-focus-ring focus-visible:ring-4 focus-visible:ring-focus-ring-muted"
              maxLength={2000}
              value={state.note}
              onChange={(event) =>
                onChange({ ...state, note: event.target.value, status: 'editing' })
              }
            />
          </label>
          {state.status === 'error' ? (
