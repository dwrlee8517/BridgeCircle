...
#
# This is a developer script. CI does the same work as discrete steps in
# e2e.yml instead of calling it, because that workflow authenticates Docker
# Hub and wraps `supabase start` in a bounded retry against anonymous-pull
# throttling — Actions-specific hardening with no local equivalent. The CI
# branches below are kept and still exercised by hand (`CI=1`): they are what
# a future consolidation would build on, and they differ only where the
# environment does — Doppler auth from a DOPPLER_TOKEN service token (no login
# check, no directory scoping, the token already pins project/config), the
# Docker daemon a given, Studio and the log pipeline excluded, and Playwright
# installing system deps. The dev-stage integ suite in cd.yml is out of scope
