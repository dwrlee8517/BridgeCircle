...
> **Product-model notice (2026-07-13):** This file preserves the original
> launch-cut sequence and the current legacy backend contract. The approved
...
> and the approved, not-yet-implemented schema target is the
...
It is a strict narrowing of `spec.md`. The full Phase 1 vision is unchanged — this doc only specifies what ships first.
...
- no field-level visibility UI at launch — ship with hardcoded defaults from `spec.md` (name/year/city/employer/title/university/major org-visible; contact links friends-only)
