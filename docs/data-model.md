...
This document explains the schema that ships in `0001_init.sql`. It covers every table from [phase-1-launch-spec.md](phase-1-launch-spec.md) "Data Model At Launch" and the rationale behind each design choice.
...
Per [phase-1-launch-spec.md:131](phase-1-launch-spec.md): keep the split in schema even though UI shows one combined profile until org #2 onboards.
...
- Per [phase-1-launch-spec.md:24](phase-1-launch-spec.md), valid invite token = auto-approve, so `invite.accepted` triggers `organization_membership(status='active')` directly.
...
Fields per [phase-1-launch-spec.md:50](phase-1-launch-spec.md):
...
- `profile_refresh_prompts`: per-org-membership, due-date driven (the 6–12 month freshness loop from [phase-1-spec.md:284](phase-1-spec.md))
...
The `resumes` Supabase Storage bucket is already provisioned for week 3 per [phase-1-launch-spec.md:158](phase-1-launch-spec.md). `resume_uploads` is the metadata table that points into it.
...
`updated_at` = last change. `confirmed_at` = user said "yes, still correct." These diverge in a useful way: clicking "confirm" without editing advances `confirmed_at` but not `updated_at`. That distinction powers the **profile freshness rate** metric in [phase-1-spec.md:515](phase-1-spec.md).

### Background updates need a proposal inbox

LinkedIn sync, resume re-parse, employer-domain heuristics — none of these should silently overwrite the user's data. The product principle in [phase-1-spec.md:289](phase-1-spec.md) is explicit: external profile imports must support user confirmation.
...
Once `work_history` and `education_history` exist, the launch spec ranking from [phase-1-launch-spec.md:39](phase-1-launch-spec.md) gets a quiet upgrade:
