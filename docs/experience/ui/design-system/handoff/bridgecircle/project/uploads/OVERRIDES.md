...
| E3 | Desktop / wide viewport | **desktop token layer + 3 specimens** — breakpoints 768/1024/1280; containers `--container-reading 680` / `--container-shell 1140`; **sidebar shell** (240px rail, identity dots, `--sidebar-width[-rail]`); pointer states (`--hover-tint`, `--row-hover`, `--nav-active-*` — hover is a background shift, never a lift); operator **data table** (`--table-row-height 44`, header bg, sortable/hover/pagination); two-column list+detail; wide band hero (O3 × O4 at shell width); `:focus-visible` reuses `--focus-ring`. ALL of it is invention in the Toss spirit — TDS ships no desktop — seeded from production's density modes / admin tables / sidebar. Nav decision: **sidebar for the member shell**, top-nav reserved for auth/marketing. **Redesign IA (Richard, 2026-07-05): five sections — Home (dashboard) · Help (ONE page hosting the ask/give toggle in-page; split blue/green identity dot) · People (directory) · Messages (all member communication — ADR 0011 one-inbox) · School (events + announcements).** | FL-8 (the calm turns empty at 1280px) | **applied 2026-07-05** |
