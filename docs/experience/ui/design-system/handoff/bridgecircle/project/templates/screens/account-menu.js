// bc-account-menu — the topbar avatar popover (account menu). ONE copy: it is
// mounted by Shell.dc.html, so every screen gets the same menu for free. The
// DC-native duplicates that Home and AppShell used to carry are gone.
//
// Attributes (all optional): name, email, initials, base (href prefix,
// default "." — templates/screens/ is flat).
    ...
    connectedCallback() {
      ...
      root.innerHTML = `
        <style>
          ...
          .sw{width:34px;height:20px;border-radius:999px;position:relative;border:0;padding:0;cursor:pointer;flex:none;background:var(--icon-muted)}
          ...
          .sw.on i{left:16px}
          .foot{display:flex;align-items:center;gap:14px;padding:11px 18px 13px;border-top:1px solid var(--divider-row);background:var(--surface-thread)}
        ...
        <div class="menu" role="dialog" aria-label="Account" hidden>
          <a class="id" href="${base}/ProfileSelf.dc.html">
            <span class="av">${initials}</span>
            <span style="flex:1;min-width:0"><b>${name}</b><small>${email}</small></span>
            <span class="chev">&rsaquo;</span>
          </a>
          <span class="items">
            <a href="${base}/ProfileSelf.dc.html">View profile</a>
            <a href="${base}/MyCircle.dc.html">My circle</a>
            <a href="${base}/AskHistory.dc.html">Your asks</a>
            <a href="${base}/Notifications.dc.html">Notifications</a>
            <a href="${base}/Settings.dc.html">Settings</a>
          </span>
          <span class="pause">
          ...
          <span class="foot">
            ...
            <a class="out" href="${base}/SignedOut.dc.html">Sign out</a>
          </span>
        </div>`;
